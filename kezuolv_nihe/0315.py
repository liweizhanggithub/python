...
def data_0315(o1, d1):
    ...
    """ 1、连接oracle，进行查询"""
    oracle = oc.Oracle()
    df = oracle.query_data(sql_0315 % (o1, d1))
    ...
    column1 = ['ONSTATION', 'OFFSTATION','TRAINNO','BOARDDATE',
               'ONTIME','OFFTIME','KEZUOLV']
...
if __name__ == '__main__':
    ...
    for data in od_data.values:
        try:
            cc = data_0315(data[0], data[1])
            print cc
            cc.to_excel(u'G:\\zlw_paper_new\\拟合数据\\有验证\\0315\\%sto%sto0315.xlsx' %
                (data[0], data[1]),sheet_name='0315')
